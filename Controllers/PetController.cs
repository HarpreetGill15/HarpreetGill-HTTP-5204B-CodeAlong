...
namespace PetGrooming.Controllers
{
    public class PetController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Update(string PetName, string PetColor, double PetWeight,string PetNotes,int id,int speciesID)
        {

            Debug.WriteLine("I am trying to edit a pet's name to "+PetName+" and change the weight to "+PetWeight.ToString());

            string query = "update pets set PetName = @PetName, Weight = @PetWeight, SpeciesID = @SpeciesId,  color = @PetColor, Notes = @PetNotes where PetId = "+id;
            SqlParameter[] sqlparams = new SqlParameter[5]; //0,1,2,3 pieces of information to add
            //each piece of information is a key and value pair
            sqlparams[0] = new SqlParameter("@PetName", PetName);
            sqlparams[1] = new SqlParameter("@PetWeight", PetWeight);
            sqlparams[2] = new SqlParameter("@SpeciesId", speciesID);
